...
export default function RecordButton({ state, onClick }: RecordButtonProps) {
  ...
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className={`relative flex h-10 w-10 shrink-0 items-center justify-center rounded-full ring-2 ${c.bg} ${c.ring} text-white transition-all disabled:cursor-not-allowed disabled:opacity-50`}
    >
      {c.animate && (
