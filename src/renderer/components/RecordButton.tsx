...
export default function RecordButton({ state, onClick }: RecordButtonProps) {
  ...
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className={`relative flex h-20 w-20 items-center justify-center rounded-full ring-4 ${c.bg} ${c.ring} text-white transition-all disabled:cursor-not-allowed disabled:opacity-50`}
    >
      {c.animate && (
